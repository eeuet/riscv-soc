...
// MT import freechips.rocketchip.util.PlusArg
...
class UARTTx(c: UARTParams) extends Module {
  ...
  // MT val plusarg_tx = PlusArg("uart_tx", 1, "Enable/disable the TX to speed up simulation").orR
...
/*  when (io.in.fire()) {
  ...
  when (io.in.ready) {  // MT io.in.fire() && plusarg_tx
    shifter := Cat(io.in.bits, Bits(0, 1))
    counter := Mux1H((0 until c.stopBits).map(i =>
      (io.nstop === UInt(i)) -> UInt(n + i + 2)))
  }
  when (busy) {
    prescaler := Mux(pulse, (io.div - 1.U), prescaler - UInt(1))
  }
  when (pulse && busy) {
